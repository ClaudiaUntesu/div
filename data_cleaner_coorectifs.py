...
def transcodification_bdf(X: DataFrame):
    ...
    # Conversion de la date de comparaison en Timestamp
    date_comparaison = to_datetime("2022-01-08")

    # transcodification
    if notna(X["DATE_ARRETE_BDF"]):
        if X["DATE_ARRETE_BDF"] >= date_comparaison:
            ...
            # correctif à faire dans la PROD
            if X["COTECREDIT"] == "X0":
                note_bdf_transco = "X0"
...
class dataCleaner(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        # AJOUT DE TRANSFORM OLD DATA_CLEANER: Gestion des colonnes manquantes
        missing_var = [c for c in self.features if c not in X.columns]
        X[missing_var] = numpy.nan

        # correction des valeurs en manquantes en NaN
        X.loc[(X['EVOLFONDSPROPRESSURBILAN']==-999) & (X['NBBILANS']==1), 'EVOLFONDSPROPRESSURBILAN'] = numpy.nan
        X.loc[(X['EVOLRESULTATNETANNUALISE']==-999) & (X['NBBILANS']==1), 'EVOLRESULTATNETANNUALISE'] = numpy.nan
        X.loc[(X['EVOLTRESORERIESURJCAANNUEL']==-999) & (X['NBBILANS']==1), 'EVOLTRESORERIESURJCAANNUEL'] = numpy.nan
        X.loc[(X['EVOLCA']==-999) & (X['NBBILANS']==1), 'EVOLCA'] = numpy.nan

        # convertir la note bien prinicipal en variable categorielle
        X['NOTEBIENPRINCIPAL'] = X['NOTEBIENPRINCIPAL'].astype(str)
        ...
        X["DATE_ARRETE_BDF"] = to_datetime(X["DATE_ARRETE_BDF"])
        # for date in dates:
        #     delta = "".join(["DIF", date])
        #     X[delta] = (
        ...
        X = X.rename(
            {   
                "ANCIENNETE_NOTEANADEFINOR": "ANCIENNETENOR",
                "ANCIENNETE_NOTEANADEFINOFC": "ANCIENNETENOFC",
                "ANCIENNETE_NOTESRR": "ANCIENNETESRR",
                "IRPRO": "NOTE_IRPRO",
        ...
        mask_outlier_1 = X["EXPOPOTENTIELLEBRUTEGROUPE"] < 0
        X.loc[mask_outlier_1, "EXPOPOTENTIELLEBRUTEGROUPE"] = 0
        mask_outlier_2 = X["ANCIENNETECONTREPARTIE"] < 0
        X.loc[mask_outlier_2, "ANCIENNETECONTREPARTIE"] = nan

        mask_outlier_3 = X["MTIMP3MOISCTP"] < 0
        X.loc[mask_outlier_3, "MTIMP3MOISCTP"] = 0

        mask_outlier_4 = X["ENCOURSMAX24MOISGROUPE"] < 0
        X.loc[mask_outlier_4, "ENCOURSMAX24MOISGROUPE"] = 0

        # AJOUT DE TRANSFORM OLD DATA_CLEANER: Correction du montant unitaire du bien principal
        mask = X['MONTANTUNITAIREBIENPRINCIPAL'] > X['MTDEMANDE']
        X.loc[mask, 'MONTANTUNITAIREBIENPRINCIPAL'] = X.loc[mask, 'MTDEMANDE']
        mask = X['MONTANTUNITAIREBIENPRINCIPAL'] == 0
        X.loc[mask, 'MONTANTUNITAIREBIENPRINCIPAL'] = X.loc[mask, 'MTDEMANDE']
        ...
        # AJOUT DE TRANSFORM OLD DATA_CLEANER: Ratios supplémentaires
        if all(col in X.columns for col in ['DETTESINF1AN', 'CAPACITEAUTOFINANCEMENT']):
            X['DETTESINF1AN_CAPACITEAUTOFINANCEMENT'] = X['DETTESINF1AN'] / X['CAPACITEAUTOFINANCEMENT'].replace(0, nan)
        
        if all(col in X.columns for col in ['BESOINFONDSDEROULEMENT', 'TRESORERIE']):
            X['BESOINFONDSDEROULEMENT_TRESORERIE'] = X['BESOINFONDSDEROULEMENT'] / X['TRESORERIE'].replace(0, nan)
        
        if all(col in X.columns for col in ['COURTTERME', 'TRESORERIE']):
            X['COURTTERME_TRESORERIE'] = X['COURTTERME'] / X['TRESORERIE'].replace(0, nan)
        
        if all(col in X.columns for col in ['EVOLCA', 'BESOINFONDSDEROULEMENT']):
            X['EVOLCA_BESOINFONDSDEROULEMENT'] = X['EVOLCA'] / X['BESOINFONDSDEROULEMENT'].replace(0, nan)
        
        if all(col in X.columns for col in ['COURTTERME', 'AUTRESCOURTTERME', 'TOTALACTIF']):
            X['TOTALCOURTTERME_TOTALACTIF'] = (X['COURTTERME'] + X['AUTRESCOURTTERME']) / X['TOTALACTIF'].replace(0, nan)
        
        if all(col in X.columns for col in ['BESOINFONDSDEROULEMENT', 'FONDSDEROULT']):
            X['BESOINFONDSDEROULEMENT_FONDSDEROULT'] = X['BESOINFONDSDEROULEMENT'] / X['FONDSDEROULT'].replace(0, nan)
        ...
        # AJOUT DE TRANSFORM OLD DATA_CLEANER: Construction de la catégorie experte
        if 'CODENATUREBIENPRINCIPAL' in X.columns:
            mask = X['CODENATUREBIENPRINCIPAL'].isin(['R19P','R20P','R23S','R20W','R20R','R20S','R20T','R20U',
                                                'R20Q','R16P','R19Q','R21S','R22S','R24S','R23S','R20W','R25S','R26S','R27S','R19Q'])
            X['CODECATEGORIEBIENPRINCIPAL_EXPERT'] = X['CODECATEGORIEBIENPRINCIPAL']
            X['CODENATUREBIENPRINCIPAL_EXPERT'] = X['CODENATUREBIENPRINCIPAL']
            X.loc[mask, 'CODECATEGORIEBIENPRINCIPAL_EXPERT'] = 'R1920P_expert'
            X.loc[mask, 'CODENATUREBIENPRINCIPAL_EXPERT'] = 'R1920P_expert'
        ...
        # AJOUT DE TRANSFORM OLD DATA_CLEANER: Calcul du ratio MTENCOURSNOTE5CONTREPARTIE_MTENCOURS
        X['MTENCOURSNOTE5CONTREPARTIE_MTENCOURS'] = (X['MTENCOURSNOTE5CONTREPARTIE'] / X['MTENCOURS'].replace(0, nan)).fillna(0)
        ...
        # AJOUT DE TRANSFORM OLD DATA_CLEANER: Calcul du ratio MTENCOURSFUTUR_CAPACITEAUTOFINANCEMENT
        if 'CAPACITEAUTOFINANCEMENT' in X.columns:
            X['MTENCOURSFUTUR_CAPACITEAUTOFINANCEMENT'] = X['MTENCOURSFUTUR'] / X['CAPACITEAUTOFINANCEMENT'].replace(0, nan)

        # AJOUT DE TRANSFORM OLD DATA_CLEANER: Correction des valeurs aberrantes pour MTENCOURSFUTUR_FONDSPROPRES
        mask = X['MTENCOURSFUTUR_FONDSPROPRES'] > 9.312703
        X.loc[mask, 'MTENCOURSFUTUR_FONDSPROPRES'] = 0
        mask = X['MTENCOURSFUTUR_FONDSPROPRES'] < -0.009010
        X.loc[mask, 'MTENCOURSFUTUR_FONDSPROPRES'] = 0
        
        # AJOUT DE TRANSFORM OLD DATA_CLEANER: Correction des valeurs aberrantes pour ANCIENNETECONTREPARTIE
        mask = X['ANCIENNETECONTREPARTIE'] > 769
        X.loc[mask, 'ANCIENNETECONTREPARTIE'] = nan
